...
# There are many plots used to guide the developemnt
...
pred5 <- predict(fit5,newdata)
plot(pred5,LUTotGHGbyRevA)

result <- cbind(TotalGHG, Revenue, LUTotGHGbyRevA, pred5)
result[!is.na(TotalGHG),][1:10,]
colnames(result) <- c("TotalGHG", "Revenue", "log(GHG/Rev)","Predicted")
resultfile <- paste(CurrentDirectory,"/Fits.csv",sep="")
write.csv(file=resultfile, result)